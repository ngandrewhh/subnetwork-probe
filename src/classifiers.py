...
class UDModel(nn.Module):
    ...
    def predict_batch(self, sentences, correct_heads):
        ...
        dep_W_headT.data.masked_fill_(pad_mask.to(torch.bool), -float('inf'))
...
class SPAMModel(nn.Module):
    def __init__(self, bert_encoder):
        ...
        self.bert = self.bert_encoder.bert
        d_features = self.bert_encoder.bert.dim
        self.out = nn.Linear(d_features, 2)
    ...
    def predict_batch(self, sentences):
        ...
        enc_unpacked, enc_mask = unpack(enc, list(map(len, sentences)))
        enc_unpacked = self.out(enc_unpacked)
        return enc_unpacked, enc_mask
